...
uint8_t prev_controlSignal=5,controlSignal=5;
...
{
    prev_firstTimeValue=firstTimeValue=0.0;
    globalAdelay_flag=1;
    prev_controlSignal_flag=5;
    time(&previousTime);
}
...
uint8_t EETCC_controlSignal_v2(double PMV1,double PMV2,double PMV3,double PMV4,uint8_t prev_EETCC_controlSignal,float timer,uint8_t index,float local_mean_air_velocity,float temperature_mean_radiation,float temperature_outdoor,float solar_radiation,float GlobalA,float GlobalA_delay)
{
    ...
    //modified EETCC with season 23/8/2016
    if(EETCC_Season=='1')   //spring
    {
        //modified EETCC    16/5/2017
        if(prev_EETCC_controlSignal==1 || prev_EETCC_controlSignal==2 || prev_EETCC_controlSignal==3 || prev_EETCC_controlSignal==4)
        {
            if(temperature_outdoor>=temperature_mean_radiation)
                timesh=(SUMMER_MINIMUM_TIME_WITHOUT_USING_AIRCOND_ToutHI*6);
            else
                timesh=(SUMMER_MINIMUM_TIME_WITHOUT_USING_AIRCOND_ToutLOW*6);
        }
        else
        {
            if(temperature_outdoor>=temperature_mean_radiation)
                timesh=(SUMMER_MINIMUM_TIME_USING_AIRCOND_ToutHI*6);
            else
                timesh=(SUMMER_MINIMUM_TIME_USING_AIRCOND_ToutLOW*6);
        }
        //end of modified EETCC 16/5/2017
    }
    else if(EETCC_Season=='2')    //summer
    {
    ...
    //end of modified EETCC with season 23/8/2016
    ...
    else
    {
        if(timer>timesh)
        {
            ...
            //modified EETCC with season 16/5/2017
            if(EETCC_Season=='1')
            {
                //modified EETCC    16/5/2017
                if(PMV1<=-0.5)
                ...
                else if(PMV>=0.5 && (temperature_outdoor>=temperature_mean_radiation))
                {
                    if(control==1 || control==2 || control==3 || control==4 || control==6)
                        control=5;
                }
                else if((prev_EETCC_controlSignal==5 || prev_EETCC_controlSignal==6) && (control==3 || control==4))
                {
                    control=1;
                }
                //end of modified EETCC 16/5/2017
...
/*
 EETCC Season
 Note: 1) Used to pass season value into EETCC algorithm
 */
void EETCC_season(char season)
{
    EETCC_Season=season;
}
