...
//
//  start of modification 15 May 2017 ####################################################################
//

uint8_t tempUDPnetworkfatal1;
uint8_t tempUDPnetworkfatal2;
uint8_t tempUDPnetworkfatal3;
uint8_t tempUDPnetworkfatal4;
uint8_t tempUDPnetworkfatal5;
float tempsensor_AirSpeed1;
float tempsensor_AirSpeed2;
float tempsensor_AirSpeed3;
float tempsensor_AirSpeedOutdoor;
float tempsensor_TemperatureOutdoor;
float tempsensor_HumidityOutdoor;
float tempsensor_TemperatureIndoor;
float tempsensor_HumidityIndoor;
uint8_t tempstate_aircond;
float tempaircond_current_consumption;
uint8_t tempaircond_room_humidity;
int8_t tempaircond_room_temperature;
int8_t tempaircond_cooled_air_temperature;
int8_t tempaircond_outdoor_air_temperature;
...
int main(void)
{
    uint8_t specialcounter=0;
    fp=fopen(filename,"a+");
    time(&rawtime);
    fprintf(fp,"\n%s \n%s \n%s","-------------------------",ctime(&rawtime),"\n-------------------------");
    fprintf(fp,"\ntempsensor_AirSpeed1, tempsensor_AirSpeed2, tempsensor_AirSpeed3, tempsensor_AirSpeedOutdoor, tempsensor_TemperatureOutdoor, tempsensor_HumidityOutdoor, tempsensor_TemperatureIndoor, tempsensor_HumidityIndoor, tempstate_aircond, tempaircond_current_consumption, tempaircond_room_humidity, tempaircond_room_temperature, tempaircond_cooled_air_temperature, tempaircond_outdoor_air_temperature, tempstate_window1, tempstate_window2, tempstate_curtain, timestamp");
    fclose(fp);
    //Init. setup
    ...
    while(iOS_status=='0')
    {
        ...
        else if(iOScommand[0]=='2' && UDPpacket_length==1)
        {
            ...
            printf("Debug point iOS status1\n");
            //echonetMT_getiHouse_2_202(10); //3 Indoor Airspeed sensor (IP:192.168.2.202) TID: 10
            echonet_getiHouse_2_202(10,(float *)&tempsensor_AirSpeed1,(float *)&tempsensor_AirSpeed2,(float *)&tempsensor_AirSpeed3,(unsigned char*)&tempUDPnetworkfatal1);
            printf("\n202: airspeed1: %f airspeed2: %f airspeed3: %f",tempsensor_AirSpeed1,tempsensor_AirSpeed2,tempsensor_AirSpeed3);
            
            //echonetMT_getiHouse_2_145(20); //1 Outdoor Airspeed sensor,1 Outdoor Temperature sensor,1 Outdoor Humidity sensor (IP:192.168.2.145) TID: 20
            echonet_getiHouse_2_145(20,(float *)&tempsensor_AirSpeedOutdoor,(float *)&tempsensor_TemperatureOutdoor,(float *)&tempsensor_HumidityOutdoor,(unsigned char*)&tempUDPnetworkfatal1,(unsigned char*)&tempUDPnetworkfatal2);
            printf("\n145: airspeedoutdoor: %f temperatureoutdoor: %f humidityoutdoor: %f",tempsensor_AirSpeedOutdoor, tempsensor_TemperatureOutdoor,tempsensor_HumidityOutdoor);
            
            //echonetMT_getiHouse_2_108(30); //1 Indoor Temperature sensor,1 Indoor Humidity sensor (IP:192.168.2.108) TID: 30
            echonet_getiHouse_2_108(30,(float *)&tempsensor_TemperatureIndoor,(float *)&tempsensor_HumidityIndoor,(unsigned char*)&tempUDPnetworkfatal1);
            printf("\n108: temperatureindoor: %f humidityindoor: %f",tempsensor_TemperatureIndoor,tempsensor_HumidityIndoor);
            
            //echonetMT_getiHouse_2_166(50); //2 Switch, Window1 (IP:192.168.2.166) TID: 50
            echonet_getiHouse_2_166(50,(unsigned char*)&tempstate_window1,(unsigned char*)&tempUDPnetworkfatal1);
            printf("\n166: statewindow1: %d",tempstate_window1);
            
            //echonetMT_getiHouse_2_167(60); //2 Switch, Window2 (IP:192.168.2.167) TID: 60
            echonet_getiHouse_2_167(60,(unsigned char*)&tempstate_window2,(unsigned char*)&tempUDPnetworkfatal1);
            printf("\n167: statewindow2: %d",tempstate_window2);
            
            //echonetMT_getiHouse_2_158(70); //2 Switch, Curtain (IP:192.168.2.158) TID: 70
            echonet_getiHouse_2_158(70,(unsigned char*)&tempstate_curtain,(unsigned char*)&tempUDPnetworkfatal1);
            printf("\n158: statecurtain: %d",tempstate_curtain);
            
            //echonetMT_getiHouse_2_174(40); //Air Conditioner (IP:192.168.2.174) TID: 40
            echonet_getiHouse_2_174(40,(unsigned char*)&tempstate_aircond,(float*)&tempaircond_current_consumption,(unsigned char*)&tempaircond_room_humidity,(unsigned char*)&tempaircond_room_temperature,(unsigned char*)&tempaircond_cooled_air_temperature,(unsigned char*)&tempaircond_outdoor_air_temperature,(unsigned char*)&tempUDPnetworkfatal1);
            printf("\n174: stateaircond: %d",tempstate_aircond);
            ...
            fp=fopen(filename,"a+");
            fprintf(fp,"\n%f,%f,%f,%f,%f,%f,%f,%f,%d,%f,%d,%d,%d,%d,%d,%d,%d,%s",tempsensor_AirSpeed1,tempsensor_AirSpeed2,tempsensor_AirSpeed3,tempsensor_AirSpeedOutdoor,tempsensor_TemperatureOutdoor,tempsensor_HumidityOutdoor,tempsensor_TemperatureIndoor,tempsensor_HumidityIndoor,tempstate_aircond,tempaircond_current_consumption,tempaircond_room_humidity,tempaircond_room_temperature,tempaircond_cooled_air_temperature,tempaircond_outdoor_air_temperature,tempstate_window1,tempstate_window2,tempstate_curtain,ctime(&rawtime));
            fclose(fp);
            printf("\nEND of Debug point\n");
            //
            //  end of modification 15 May 2017 ######################################################################
        ...
        else
        {
            ...
            printf("Debug point iOS status3\n");
        }
    }
...
void ECHONETMT_LITE_MAIN_ROUTINE(void)
{
    ...
    if(iOS_status=='1')
    {
        ...
        //3 Indoor Airspeed sensor (IP:192.168.2.202) TID: 10
        echonetMT_getiHouse_2_202(((TID_counter_local<<8) | (10 & 0xFF)));
        //Multi-thread global variable lock to prevent read/write error
        pthread_mutex_lock(&mutex);
        time(&previousTime1);
        pthread_mutex_unlock(&mutex);
        //1 Outdoor Airspeed sensor,1 Outdoor Temperature sensor,1 Outdoor Humidity sensor (IP:192.168.2.145) TID: 20
        echonetMT_getiHouse_2_145(((TID_counter_local<<8) | (20 & 0xFF)));
        //Multi-thread global variable lock to prevent read/write error
        pthread_mutex_lock(&mutex);
        time(&previousTime2);
        pthread_mutex_unlock(&mutex);
        //1 Indoor Temperature sensor,1 Indoor Humidity sensor (IP:192.168.2.108) TID: 30
        echonetMT_getiHouse_2_108(((TID_counter_local<<8) | (30 & 0xFF)));
        //Multi-thread global variable lock to prevent read/write error
        pthread_mutex_lock(&mutex);
        time(&previousTime3);
        pthread_mutex_unlock(&mutex);
        //Air Conditioner (IP:192.168.2.174) TID: 40
        echonetMT_getiHouse_2_174(((TID_counter_local<<8) | (40 & 0xFF)));
        //Multi-thread global variable lock to prevent read/write error
        pthread_mutex_lock(&mutex);
        time(&previousTime4);
        pthread_mutex_unlock(&mutex);
        //2 Switch, Window1 (IP:192.168.2.166) TID: 50
        echonetMT_getiHouse_2_166(((TID_counter_local<<8) | (50 & 0xFF)));
        //Multi-thread global variable lock to prevent read/write error
        pthread_mutex_lock(&mutex);
        time(&previousTime5);
        pthread_mutex_unlock(&mutex);
        //2 Switch, Window2 (IP:192.168.2.167) TID: 60
        echonetMT_getiHouse_2_167(((TID_counter_local<<8) | (60 & 0xFF)));
        //Multi-thread global variable lock to prevent read/write error
        pthread_mutex_lock(&mutex);
        time(&previousTime6);
        pthread_mutex_unlock(&mutex);
        //2 Switch, Curtain (IP:192.168.2.158) TID: 70
        echonetMT_getiHouse_2_158(((TID_counter_local<<8) | (70 & 0xFF)));
        //Multi-thread global variable lock to prevent read/write error
        pthread_mutex_lock(&mutex);
        time(&previousTime7);
        pthread_mutex_unlock(&mutex);
        ...
        //ControlSignal=EETCC_controlSignal_v2(PMV1,PMV2,PMV3,PMV4,prev_ControlSignal,timer,index,(sensor_AirSpeedOutdoor*0.1),sensor_TemperatureIndoor,sensor_TemperatureOutdoor,sensor_SolarRadiation,0,0); //EDITED! 16/5/2017 prevent turning on of aircond on first start due to low outdoor temperature
        ControlSignal=EETCC_controlSignal_v2(PMV1,PMV2,PMV3,PMV4,prev_ControlSignal,timer,index,(sensor_AirSpeedOutdoor*0.1),sensor_TemperatureIndoor,sensor_TemperatureOutdoor,sensor_SolarRadiation,globalA,globalA_delay); //EDITED! 16/5/2017 prevent turning on of aircond on first start due to low
        //if(sensor_Rain==0x41)
        //{
        ...
        fp=fopen(filename,"a+");
        fprintf(fp,"\n%d,%d,%d,%d,%d,%d,%f,%d,%d,%d,%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%d,%d,%d,%d,%d,%d,%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%d,%f,%d,%f,%f,%f,%f,%f,%lu,%s",(run_counter-1),state_window1,state_window2,state_curtain,state_aircond,setting_aircond_temperature,aircond_current_comsumption,aircond_room_humidity,aircond_room_temperature,aircond_cooled_air_temperature,aircond_outdoor_air_temperature,sensor_TemperatureIndoor,sensor_TemperatureOutdoor,sensor_AirSpeedIndoor,sensor_AirSpeedOutdoor,sensor_HumidityIndoor,sensor_HumidityOutdoor,sensor_SolarVoltage,sensor_SolarRadiation,UDP_network_timeout_aircond,UDP_network_timeout_airspeed_indoor,UDP_network_timeout_airspeed_outdoor,UDP_network_timeout_temperature_indoor,UDP_network_timeout_temperature_outdoor,UDP_network_timeout_window1,UDP_network_timeout_window2,UDP_network_timeout_curtain,UDP_network_fatal_aircond,UDP_network_fatal_airspeed_indoor,UDP_network_fatal_airspeed_outdoor,UDP_network_fatal_temperature_indoor,UDP_network_fatal_temperature_outdoor,UDP_network_fatal_window1,UDP_network_fatal_window2,UDP_network_fatal_curtain,PMV,PMV1,PMV2,PMV3,PMV4,PPD,PPD1,PPD2,PPD3,PPD4,ControlSignal,prev_ControlSignal,timer,index,globalA,globalA_delay,Q1,Q2,draught,Accumulative_power,ctime(&rawtime));
        fclose(fp);
        //fprintf(fp,"\ntempsensor_AirSpeed1, tempsensor_AirSpeed2, tempsensor_AirSpeed3, tempsensor_AirSpeedOutdoor, tempsensor_TemperatureOutdoor, tempsensor_HumidityOutdoor, tempsensor_TemperatureIndoor, tempsensor_HumidityIndoor, tempstate_aircond, tempaircond_current_consumption, tempaircond_room_humidity, tempaircond_room_temperature, tempaircond_cooled_air_temperature, tempaircond_outdoor_air_temperature, tempstate_window1, tempstate_window2, tempstate_curtain, timestamp");
        
        //fprintf(fp,"\n%f,%f,%f,%f,%f,%f,%f,%f,%d,%f,%d,%d,%d,%d,%d,%d,%d,%s",tempsensor_AirSpeed1,tempsensor_AirSpeed2,tempsensor_AirSpeed3,tempsensor_AirSpeedOutdoor,tempsensor_TemperatureOutdoor,tempsensor_HumidityOutdoor,tempsensor_TemperatureIndoor,tempsensor_HumidityIndoor,tempstate_aircond,tempaircond_current_consumption,tempaircond_room_humidity,tempaircond_room_temperature,tempaircond_cooled_air_temperature,tempaircond_outdoor_air_temperature,tempstate_window1,tempstate_window2,tempstate_curtain,ctime(&rawtime));
        printf("\nwindow1: %d window2: %d curtain: %d aircond: %d\nindoortemp: %f outdoortemp: %f solar: %f\nPMV: %f PMV1: %f PMV2: %f PMV3: %f PMV4: %f PPD: %f\ncontrol: %d prevcontrol: %d",state_window1,state_window2,state_curtain,state_aircond,sensor_TemperatureIndoor,sensor_TemperatureOutdoor,sensor_SolarRadiation,(float)PMV,(float)PMV1,(float)PMV2,(float)PMV3,(float)PMV4,(float)PPD,ControlSignal,prev_ControlSignal);
...
void *SERVER_PORT_LISTEN(void *threadid)
{
    ...
    while(1)
    {
        if(udp_port_listener_echonet_lite(UDPnetwork_buffer,&UDPpacket_length)==1)
        {
            ...
            if((unsigned char)UDPnetwork_buffer[0]==EHD1_ECHONET && (unsigned char)UDPnetwork_buffer[1]==EHD2_FORMAT1)
            {
                ...
                UDPpacket_TID_upper8bit=(unsigned char)(UDPpacket_TIDraw>>8);   //TAKE NOTE IF ROUND TRIP TIMING SCREWS UP!
                switch ((unsigned char)UDPnetwork_buffer[4])
                {
                    case CGC_SENSOR_RELATED:
                        switch ((unsigned char)UDPnetwork_buffer[5])
                    {
                        ...
                        case CC_TEMPERTURE_SENSOR:
                            if((unsigned char)UDPnetwork_buffer[10]==ESV_Get_Res && (unsigned char)UDPnetwork_buffer[11]==0x01)
                            {
                                if((unsigned char)UDPnetwork_buffer[12]==EPC_MEASURE_TEMPERATURE_VALUE && (unsigned char)UDPnetwork_buffer[13]==2)
                                {
                                    switch (UDPpacket_TID)
                                    {
                                        case 21:
                                            MTsensor_TemperatureOutdoor=((float)(UDPnetwork_buffer[14]<<8 | (UDPnetwork_buffer[15] & 0xFF)))/10;
                                        ...
                                        case 30:
                                            MTsensor_TemperatureIndoor=((float)(UDPnetwork_buffer[14]<<8 | (UDPnetwork_buffer[15] & 0xFF)))/10;
                                            if(UDPpacket_TID_upper8bit==TID_counter || UDPpacket_TID_upper8bit==(TID_counter-1) || UDPpacket_TID_upper8bit==245)
                                            {
                                                time(&currentTime3);
                            ...
                            else if((unsigned char)UDPnetwork_buffer[10]==ESV_Set_Res && (unsigned char)UDPnetwork_buffer[11]==0x01)
                            {
                        ...
                        case CC_HUMIDITY_SENSOR:
                            if((unsigned char)UDPnetwork_buffer[10]==ESV_Get_Res && (unsigned char)UDPnetwork_buffer[11]==0x01)
                            {
                                if((unsigned char)UDPnetwork_buffer[12]==EPC_MEASURE_HUMIDITY_VALUE && (unsigned char)UDPnetwork_buffer[13]==1)
                                {
                                    switch (UDPpacket_TID)
                                    {
                                        case 22:
                        ...
                        case CC_RAIN_SENSOR:
                            //if((unsigned char)UDPnetwork_buffer[10]==ESV_Get_Res && (unsigned char)UDPnetwork_buffer[11]==0x01)
                            //{
                                //if((unsigned char)UDPnetwork_buffer[12]==0xB1 && (unsigned char)UDPnetwork_buffer[13]==1)
                                //{
                                    //switch (UDPpacket_TID)
                                    //{
                                        //case 80:
                        ...
                        case CC_AIR_SPEED_SENSOR:
                            if((unsigned char)UDPnetwork_buffer[10]==ESV_Get_Res && (unsigned char)UDPnetwork_buffer[11]==0x01)
                            {
                                if((unsigned char)UDPnetwork_buffer[12]==EPC_MEASURE_AIR_SPEED_VALUE && (unsigned char)UDPnetwork_buffer[13]==2)
                                {
                                    switch (UDPpacket_TID)
                                    {
                                        case 10:
                                            MTsensor_AirSpeed1=((float)(UDPnetwork_buffer[14]<<8 | (UDPnetwork_buffer[15] & 0xFF)))/100;
                                            if(UDPpacket_TID_upper8bit==TID_counter || UDPpacket_TID_upper8bit==(TID_counter-1) || UDPpacket_TID_upper8bit==245)
                                            {
                                                time(&currentTime1);
                                        ...
                                        case 11:
                                            MTsensor_AirSpeed2=((float)(UDPnetwork_buffer[14]<<8 | (UDPnetwork_buffer[15] & 0xFF)))/100;
                                            if(UDPpacket_TID_upper8bit==TID_counter || UDPpacket_TID_upper8bit==(TID_counter-1) || UDPpacket_TID_upper8bit==245)
                                            {
                                                time(&currentTime1);
                                                MTUDP_network_timeout_airspeed_indoor=(float)difftime(currentTime1,previousTime1);
                                        ...
                                        case 12:
                                            MTsensor_AirSpeed3=((float)(UDPnetwork_buffer[14]<<8 | (UDPnetwork_buffer[15] & 0xFF)))/100;
                                            if(UDPpacket_TID_upper8bit==TID_counter || UDPpacket_TID_upper8bit==(TID_counter-1) || UDPpacket_TID_upper8bit==245)
                                            {
                                                time(&currentTime1);
                                                MTUDP_network_timeout_airspeed_indoor=(float)difftime(currentTime1,previousTime1);
                                        ...
                                        case 20:
                                            MTsensor_AirSpeedOutdoor=((float)(UDPnetwork_buffer[14]<<8 | (UDPnetwork_buffer[15] & 0xFF)))/100;
                                            if(UDPpacket_TID_upper8bit==TID_counter || UDPpacket_TID_upper8bit==(TID_counter-1) || UDPpacket_TID_upper8bit==245)
                                            {
                                                time(&currentTime2);
                        ...
                        switch ((unsigned char)UDPnetwork_buffer[5])
                    {
                        case CC_HOME_AIR_CONDITIONER:
                            ...
                            if((unsigned char)UDPnetwork_buffer[10]==ESV_Get_Res && (unsigned char)UDPnetwork_buffer[11]==0x01)
                            {
                                if((unsigned char)UDPnetwork_buffer[12]==EPC_OPERATIONAL_STATUS && (unsigned char)UDPnetwork_buffer[13]==1)
                                {
                        ...
                        switch ((unsigned char)UDPnetwork_buffer[5])
                    {
                        ...
                        case CC_DISTRIBUTION_PANEL_METERING:
                            accumulative_power=((unsigned char)UDPnetwork_buffer[14]<<24 | (unsigned char)UDPnetwork_buffer[15]<<16 | (unsigned char)UDPnetwork_buffer[16]<<8 | (unsigned char)UDPnetwork_buffer[17]);  //check if there is power meter value ERROR for 32bit concanate
                            break;
                        ...
                        switch ((unsigned char)UDPnetwork_buffer[5])
                    {
                        ...
                        case CC_NETWORK_CAMERA:
                            ...
                            break;
                        default:
                            printf("Input not defined in Audiovisual-related class code (0x%1X)",(unsigned char)UDPnetwork_buffer[5]);
                            break;
                    }
                        break;
            ...
            else
            {
                ...
                else if(UDPnetwork_buffer[0]=='2' && UDPpacket_length==1)
                {
                    UDPpacket_length=1;
                    return_incoming_ip(&iOS_status,&UDPpacket_length);
                    printf("Debug point iOS status1 Server Port Listen\n");
                }
                else if(UDPnetwork_buffer[0]=='3' && UDPpacket_length==1)
                {
                    UDPpacket_length=2;
                    return_incoming_ip(&iOS_season,&UDPpacket_length);
                    printf("Debug point iOS status2 Server Port Listen\n");
                    
                }
                else
                {
                    printf("Command not recognized! Server Port Listen\n");
                    int temptemptemp=0;
                    for(temptemptemp=0;temptemptemp<15;temptemptemp++)
                        printf("0x%1X ",(unsigned char)UDPnetwork_buffer[temptemptemp]);
                    printf("\n");
                }
                uint16_t echonet_TID1=246;
                switch (iOS_season)
...
void System_INT_Handler(int sig)
{
    ...
    {
        //close_sql(con);
        //make sure system is OFF before exiting
        echonetMT_setObject_operationalStatus("192.168.2.174", 10, CGC_AIR_CONDITIONER_RELATED, CC_HOME_AIR_CONDITIONER, 1, OS_OFF);   //disable air conditioner
        echonetMT_setObject_operationalStatus("192.168.2.166", 20, CGC_MANAGEMENT_RELATED, CC_SWITCH, 2, OS_ON);   //disable window1
        echonetMT_setObject_operationalStatus("192.168.2.167", 30, CGC_MANAGEMENT_RELATED, CC_SWITCH, 2, OS_ON);   //disable window2
        echonetMT_setObject_operationalStatus("192.168.2.158", 40, CGC_MANAGEMENT_RELATED, CC_SWITCH, 2, OS_ON);   //disable curtain
        echonetMT_setObject_operationalStatus("192.168.2.174", 50, CGC_AIR_CONDITIONER_RELATED, CC_HOME_AIR_CONDITIONER, 1, OS_OFF);   //disable air conditioner
        //exit system
        exit(0);
    }
    else
